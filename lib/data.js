...
// get dashboard guru piket
export const getDashboardGuruPiket = async () => {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/dashboard/piket`,
      {
        method: "GET",
        cache: "no-store",
      }
    ...
    if (!response.ok) {
      const data = await response.json();
      throw new Error(data.message || "Gagal mengambil data guru piket");
    }
